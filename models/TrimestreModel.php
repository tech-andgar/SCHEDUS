...
class TrimestreModel extends DB
{
    ...
    public function insertarTrimestresPrograma(array $data)
    {
        try {
            // Verificar hay datos en $data
            if (
                isset($data['name_trimestre']) &&
                isset($data['fecha_inicio']) &&
                isset($data['fecha_fin'])) {
                $name_trimestre = $data['name_trimestre'];
                $fecha_inicio = $data['fecha_inicio'];
                $fecha_fin = $data['fecha_fin'];
                $stm = parent::conectar()->prepare(preparedSQL::INSERT_NEW_TRIMESTRE);
                $stm->bindParam(1, $name_trimestre, PDO::PARAM_STR);
                $stm->bindParam(2, $fecha_inicio, PDO::PARAM_STR);
                $stm->bindParam(3, $fecha_fin, PDO::PARAM_STR);
                $stm->execute();
                return true;
            }
        } catch (Exception $e) {
    ...
    // TODO FIX UPDATE_DATA_TRIMESTRE
    public function updateDataTrimestresPrograma($data)
    {
        try {
            // Verificar hay datos en $data
            if (isset($data['name_trimestre']) &&
            isset($data['fecha_inicio']) &&
            isset($data['fecha_fin']) &&
            isset($data['id_trimestre'])) {
                $name_trimestre = $data['name_trimestre'];
                $fecha_inicio = $data['fecha_inicio'];
                $fecha_fin = $data['fecha_fin'];
                $id_trimestre = $data['id_trimestre'];
                $stm = parent::conectar()->prepare(preparedSQL::UPDATE_DATA_TRIMESTRE_ID);
                $stm->bindParam(1, $name_trimestre, PDO::PARAM_STR);
                $stm->bindParam(2, $fecha_inicio, PDO::PARAM_STR);
                $stm->bindParam(3, $fecha_fin, PDO::PARAM_STR);
                $stm->bindParam(4, $id_trimestre, PDO::PARAM_STR);
                $var = $stm->execute();
                var_dump($var);
                //return true;
            }
